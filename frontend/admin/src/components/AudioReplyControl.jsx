...
export default function AudioReplyControl({ audioUrl }) {
  ...
  return (
    <div className="audio-reply-controls" role="group" aria-label="Contrôles de la réponse vocale">
      <button className="chat-replay-btn" type="button" onClick={togglePlayback}>
        {playing ? "❚❚ Pause" : hasStarted ? "▶ Reprendre" : "🔊 Écouter"}
      </button>
      {hasStarted && <button className="chat-replay-btn chat-replay-btn--secondary" type="button" onClick={replay}>↻ Réécouter</button>}
    </div>
  );
}
