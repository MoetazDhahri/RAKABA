    ...
    <div className="panel">
      <div className="view-header">
        <div>
          ...
          <p className="subtitle">Toutes les actions déclenchées automatiquement ou manuellement, toutes pipelines confondues.</p>
        </div>
        ...
        <div className="journal-list" style={{ marginTop: 14 }}>
          ...
          {data.map((entry) => (
            <div className="journal-row" key={entry.log_id}>
              <span className={`journal-source journal-source--${entry.pipeline_source}`}>{entry.pipeline_source}</span>
              <div className="journal-desc">
                {entry.action_description}
                {entry.entity_id && <div className="drawer-subtitle">{entry.entity_id}</div>}
              </div>
